...
#ifdef GCC_RECTM
        new_number_of_activated_threads += 2;
#endif
...
void greentm_register_unblockable_thread() {
    pthread_mutex_lock(&thread_mutexes[thread_id].mutex);
    // TODO lacking the implementation and calls in the applications in need of this (memcached only, most likely)
    pthread_mutex_unlock(&thread_mutexes[thread_id].mutex);
}
