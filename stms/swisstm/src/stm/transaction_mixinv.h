...
inline void wlpdstm::TxMixinv::RestartJump() {
    ...
    RestartJumpInC(start_buf, LONG_JMP_RESTART_FLAG);
}

inline void wlpdstm::TxMixinv::TxAbort() {
printf("This should never be called because we do not expect user aborts. Otherwise, must be fixed in swisstm, because we are changing aborts to make the transaction ready to restart without calling begin\n");
exit(1);
...
inline void wlpdstm::TxMixinv::AbortJump() {
    RestartJumpInC(start_buf, LONG_JMP_ABORT_FLAG);
}
