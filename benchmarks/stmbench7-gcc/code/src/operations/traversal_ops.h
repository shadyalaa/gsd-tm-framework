...
namespace sb7 {
	...
	class Traversal1 : public Operation {
		...
		public:
			...
			__attribute__((transaction_safe)) virtual int run() const;

		protected:
			__attribute__((transaction_safe)) int traverse(sh_ptr<ComplexAssembly> complexAssembly) const;
			__attribute__((transaction_safe)) int traverse(sh_ptr<BaseAssembly> baseAssembly) const;
			__attribute__((transaction_safe)) virtual int traverse(sh_ptr<CompositePart> component) const;
			__attribute__((transaction_safe)) virtual int traverse(sh_ptr<AtomicPart> part,
				Set<sh_ptr<AtomicPart> > &setOfVisitedParts) const;
			__attribute__((transaction_safe)) virtual int performOperationOnAtomicPart(sh_ptr<AtomicPart> apart,
	...
	class Traversal3a : public Traversal1 {
		...
		protected:
			...
			__attribute__((transaction_safe)) void updateBuildDate(sh_ptr<AtomicPart> apart) const;
	};
	...
	class Traversal4 : public Traversal1 {
		...
		protected:
			__attribute__((transaction_safe)) virtual int traverse(sh_ptr<CompositePart> component) const;
			__attribute__((transaction_safe)) virtual int traverse(sh_ptr<Document> doc) const;
			__attribute__((transaction_safe)) virtual int traverse(sh_ptr<AtomicPart> part,
				Set<sh_ptr<AtomicPart> > &setOfVisitedParts) const;
			__attribute__((transaction_safe)) virtual int performOperationOnAtomicPart(sh_ptr<AtomicPart> apart,
				Set<sh_ptr<AtomicPart> > &setOfVisitedPartIds) const;
	...
	class Traversal7 : public Operation {
		...
		protected:
			__attribute__((transaction_safe)) int traverse(sh_ptr<CompositePart> cpart) const;
			__attribute__((transaction_safe)) int traverse(sh_ptr<Assembly> assembly,
				Set<sh_ptr<Assembly> > &visitedAssemblies) const;
			__attribute__((transaction_safe)) virtual void performOperationOnAssembly(
	...
	class Traversal8 : public Operation {
		...
		protected:
			__attribute__((transaction_safe)) virtual int traverse(sh_ptr<Manual> manual) const;
	};
