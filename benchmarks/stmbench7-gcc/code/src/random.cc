...
namespace sb7 {
	...
	__attribute__((transaction_pure)) int Random::nextInt() {
		return rand_r(&seed);
	}
	
	__attribute__((transaction_pure)) int Random::nextInt(int n) {
		return rand_r(&seed) % n;
	}
	
	__attribute__((transaction_pure)) double Random::nextDouble() {
