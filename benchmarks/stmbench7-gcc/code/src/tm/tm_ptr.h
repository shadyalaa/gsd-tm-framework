...
__attribute__((transaction_pure)) inline void sb7::ObjectLog::put(void *addr, void *clone) {
	log[addr] = clone;
}

__attribute__((transaction_pure)) inline void *sb7::ObjectLog::get(void *addr) {
	obj_log::iterator iter = log.find(addr);
	return (iter == log.end()) ? NULL : (*iter).second;
}

__attribute__((transaction_pure)) inline void sb7::ObjectLog::clear() {
	log.clear();
}

__attribute__((transaction_pure)) inline void sb7::ObjectLog::merge(ObjectLog *log_to_merge) {
...
__attribute__((transaction_pure)) inline sb7::ObjectLog *sb7::getObjectLog() {
	return (ObjectLog *)::pthread_getspecific(obj_log_key);
}

__attribute__((transaction_pure)) inline void sb7::obj_log_tx_start(ObjectLog *pl) {
	ObjectLog *log = getObjectLog();
	log->setParentLog(pl);
}

__attribute__((transaction_pure)) inline void sb7::obj_log_tx_commit() {
	ObjectLog *log = getObjectLog();
...
__attribute__((transaction_pure)) inline void sb7::obj_log_tx_abort() {
	getObjectLog()->clear();
}
...
namespace sb7 {
    ...
    template <typename T>
    template <typename T2> __attribute__((transaction_safe))
