...
long set_add(TM_ARGDECL long val)
{
	...
	if (mode == 0) {
		...
		prev = set->head;
		next = SLOW_PATH_SHARED_READ_P(prev->next);
		while (1) {
			v = SLOW_PATH_SHARED_READ(next->val);
			if (v >= val)
				break;
			prev = next;
			next = SLOW_PATH_SHARED_READ_P(prev->next);
		...
		res = (v != val);
		if (res) {
			SLOW_PATH_SHARED_WRITE_P(prev->next, new_node(val, next));
		}
	}
	TM_END();
...
int set_remove(TM_ARGDECL long val)
{
	...
	if (mode == 0) {
		...
		res = (v == val);
		if (res) {
			n = SLOW_PATH_SHARED_READ_P(next->next);
			SLOW_PATH_SHARED_WRITE_P(prev->next, n);
			SLOW_PATH_FREE(next);
		}
	}
	TM_END();
