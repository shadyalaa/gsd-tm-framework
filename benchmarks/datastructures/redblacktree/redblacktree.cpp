...
int set_add(TM_ARGDECL  intset_t *set, intptr_t val, int mode)
{
    int res = 0;

    res = mode == 0 ? !rbtree_htm::TMrbtree_insert(TM_ARG set, val, val) : !rbtree_stm::TMrbtree_insert(TM_ARG set, val, val);

    return res;
}

int set_remove(TM_ARGDECL intset_t *set, intptr_t val, int mode)
{
    int res = 0;

    res = mode == 0 ? rbtree_htm::TMrbtree_delete(TM_ARG set, val) : rbtree_stm::TMrbtree_delete(TM_ARG set, val);

    return res;
}

int set_contains(TM_ARGDECL  intset_t *set, intptr_t val, int mode)
{
    int res = 0;

    res = mode == 0 ? rbtree_htm::TMrbtree_contains(TM_ARG set, val) : rbtree_stm::TMrbtree_contains(TM_ARG set, val);
...
#ifdef CHANGINGWORKLOAD
      if (thread_id == 0 && combinationsValue){
      ...
      for (access = 0; access < accessesPerOperations; access++) {
          ...
          if (val < update) {
              if (val < update / 2) {
                  ...
                  set_add(TM_ARG set, val, mode);
              } else {
                  /* Remove random value */
                  val = (random_generate(randomPtr) % pruned_range) + 1;
                  set_remove(TM_ARG set, val, mode);
              }
          } else {
              /* Look for random value */
              long tmp = (random_generate(randomPtr) % pruned_range) + 1;
              set_contains(TM_ARG set, tmp, mode);
          }
