...
long hm_insert_stm(TM_ARGDECL List* set, long val)
{
        ...
        while (curr != NULL) {
                if (SLOW_PATH_SHARED_READ(curr->m_val) >= val)
                        break;
                prev = curr;
                curr = SLOW_PATH_SHARED_READ_P(prev->m_next);
        }

        if (!curr || (SLOW_PATH_SHARED_READ(curr->m_val) > val)) {
                Node_HM* insert_point = (Node_HM*)(prev);
...
long hm_lookup_stm(TM_ARGDECL List* set, long val)
{
	int found = 0;
	const Node_HM* curr = set->sentinel;
	curr = SLOW_PATH_SHARED_READ_P(curr->m_next);
	...
	found = ((curr != NULL) && (SLOW_PATH_SHARED_READ(curr->m_val) == val));
	return found;
}
...
int hm_remove_stm(TM_ARGDECL List* set, long val)
{
	...
	while (curr != NULL) {
		...
		if (SLOW_PATH_SHARED_READ(curr->m_val) == val) {
			Node_HM* mod_point = (Node_HM*)(prev);
			SLOW_PATH_SHARED_WRITE_P(mod_point->m_next, SLOW_PATH_SHARED_READ_P(curr->m_next));

			SLOW_PATH_FREE((Node_HM*)(curr));
			return 1;
		}
		else if (SLOW_PATH_SHARED_READ(curr->m_val) > val) {
			return 0;
		}
		prev = curr;
		curr = SLOW_PATH_SHARED_READ_P(prev->m_next);
...
long set_add(TM_ARGDECL long val)
{
    ...
    int mode = 0;
    TM_BEGIN(0, mode);
    res = mode == 0 ? priv_insert_htm(TM_ARG val) : priv_insert_stm(TM_ARG val);
    TM_END();
...
int set_remove(TM_ARGDECL long val)
{
    ...
    int mode = 0;
    TM_BEGIN(1, mode);
    res = mode == 0 ? priv_remove_item_htm(TM_ARG val) : priv_remove_item_stm(TM_ARG val);
    TM_END();
...
long set_contains(TM_ARGDECL long  val)
{
    ...
    res = mode == 0 ? priv_lookup_htm(TM_ARG val) : priv_lookup_stm(TM_ARG val);
    TM_END();
